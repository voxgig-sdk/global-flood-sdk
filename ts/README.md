# GlobalFlood TypeScript SDK



The TypeScript SDK for the GlobalFlood API — a type-safe, entity-oriented client with full async/await support.

The API is exposed as capitalised, semantic **Entities** — e.g.
`client.Flood()` — each with a small set of operations (`load`)
...
```ts
const client = new GlobalFloodSDK({ apikey: '...' })
...
await entity.load({ latitude: 'example_latitude', longitude: 'example_longitude' })
...
| `tester(testopts?, sdkopts?)` | `GlobalFloodSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `GlobalFloodSDK.test(testopts?, sdkopts?)` | `GlobalFloodSDK` | Create a test-mode client. |
...
| `client` | `client(): GlobalFloodSDK` | Return the parent SDK client. |
...
```ts
const flood = await client.Flood().load({ latitude: 'latitude', longitude: 'longitude' })
```
...
global-flood/
├── src/
│   ├── GlobalFloodSDK.ts        # Main SDK class
...
await flood.load({ latitude: "example", longitude: "example" })
