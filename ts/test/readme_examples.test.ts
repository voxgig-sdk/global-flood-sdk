...
//     (\`new GlobalFloodSDK(...)\` / \`GlobalFloodSDK.test(...)\` become
//     \`GlobalFloodSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"flood":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new GlobalFloodSDK(...)\` and any
// \`GlobalFloodSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+GlobalFloodSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/GlobalFloodSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
