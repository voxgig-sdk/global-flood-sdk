...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.GLOBALFLOOD_TEST_LIVE ||
    'TRUE' === process.env.GLOBALFLOOD_TEST_OVERRIDE
  ) {
  ...
  m.GLOBALFLOOD_TEST_EXPLAIN = process.env.GLOBALFLOOD_TEST_EXPLAIN || m.GLOBALFLOOD_TEST_EXPLAIN
