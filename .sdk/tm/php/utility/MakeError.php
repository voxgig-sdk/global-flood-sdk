...
class GlobalFloodMakeError
{
    public static function call(?GlobalFloodContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new GlobalFloodContext([], null);
        }
        $op = $ctx->op ?? new GlobalFloodOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new GlobalFloodResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof GlobalFloodError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new GlobalFloodError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof GlobalFloodError) {
