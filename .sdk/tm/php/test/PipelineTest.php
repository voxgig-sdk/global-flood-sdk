...
{
    private static function utility(): GlobalFloodUtility
    {
        return new GlobalFloodUtility();
    }
    ...
    private static function ctx(array $over = []): GlobalFloodContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new GlobalFloodContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new GlobalFloodOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof GlobalFloodError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new GlobalFloodOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = GlobalFloodMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new GlobalFloodOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = GlobalFloodMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new GlobalFloodOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = GlobalFloodMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new GlobalFloodSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = GlobalFloodMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new GlobalFloodResponse([]),
            'result' => new GlobalFloodResult([]),
        ]);
        [, $err] = GlobalFloodMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new GlobalFloodSpec([]),
            'response' => null,
            'result' => new GlobalFloodResult([]),
        ]);
        [, $err] = GlobalFloodMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new GlobalFloodSpec(['step' => 's']),
            'response' => new GlobalFloodResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new GlobalFloodResult([]),
        ]);
        [, $err] = GlobalFloodMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new GlobalFloodSpec(['step' => 's']),
            'response' => new GlobalFloodResponse(self::resp(200, ['v' => 1])),
            'result' => new GlobalFloodResult([]),
        ]);
        [, $err] = GlobalFloodMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new GlobalFloodSpec(['step' => 's']),
            'response' => new GlobalFloodResponse(self::resp(200, ['v' => 2])),
            'result' => new GlobalFloodResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        GlobalFloodMakeResponse::call($ctx);
    ...
    {
        $preset = new GlobalFloodResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new GlobalFloodSpec([]),
            'response' => new GlobalFloodResponse([]),
            'result' => new GlobalFloodResult([]),
        ...
        [$got, $err] = GlobalFloodMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new GlobalFloodResult([])]);
        [, $err] = GlobalFloodMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new GlobalFloodSpec([]), 'result' => null]);
        [, $err] = GlobalFloodMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new GlobalFloodSpec(['step' => 's']),
            'result' => new GlobalFloodResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new GlobalFloodOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = GlobalFloodMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new GlobalFloodSpec(['step' => 's']),
            'result' => new GlobalFloodResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new GlobalFloodOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = GlobalFloodMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new GlobalFloodResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new GlobalFloodSpec([]),
            'result' => new GlobalFloodResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = GlobalFloodMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = GlobalFloodMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new GlobalFloodError('boom', 'boom');
        $utility->fetcher = function (GlobalFloodContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new GlobalFloodSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = GlobalFloodMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (GlobalFloodContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = GlobalFloodMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(GlobalFloodResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        GlobalFloodMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (GlobalFloodContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new GlobalFloodSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = GlobalFloodMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new GlobalFloodResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new GlobalFloodSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = GlobalFloodMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = GlobalFloodMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = GlobalFloodMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new GlobalFloodResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, GlobalFloodDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new GlobalFloodResult(['ok' => false])]);
        $this->expectException(GlobalFloodError::class);
        GlobalFloodDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new GlobalFloodResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', GlobalFloodMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        GlobalFloodMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new GlobalFloodBaseFeature();
        $b = new GlobalFloodBaseFeature();
        GlobalFloodFeatureAdd::call($ctx, $a);
        GlobalFloodFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): GlobalFloodBaseFeature
    {
        $f = new GlobalFloodBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        GlobalFloodFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): GlobalFloodContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new GlobalFloodSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = GlobalFloodPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        GlobalFloodPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new GlobalFloodResponse(['headers' => null]),
            'result' => new GlobalFloodResult([]),
        ]);
        GlobalFloodResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        GlobalFloodResultBody::call($ctx);
