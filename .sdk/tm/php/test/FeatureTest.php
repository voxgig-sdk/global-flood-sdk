...
class FtCtrl extends GlobalFloodControl
{
...
class FtClient
{
    ...
    private GlobalFloodUtility $utility;

    public function __construct(GlobalFloodUtility $utility, array $options)
    {
    ...
    public function get_utility(): GlobalFloodUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (GlobalFloodContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public GlobalFloodContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (GlobalFloodContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = GlobalFloodConfig::make_feature($name);
    ...
    private static function buildUrl(GlobalFloodSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof GlobalFloodError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new GlobalFloodResponse($fetched);
        }
    ...
    private function fail(GlobalFloodContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (GlobalFloodContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
