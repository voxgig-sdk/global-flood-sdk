...
boru-driven command-line client **and** interactive REPL for the GlobalFlood
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/global-flood-cli)
make build

# 2. See usage (words, entities, env vars)
./global-flood-cli --help

# 3. Provide credentials once, via the environment
export GLOBAL_FLOOD_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./global-flood-cli load 1 flood            # {id:1} shorthand
./global-flood-cli load '{id:1}' flood       # explicit match map

# 5. Override the API base URL for a single call
GLOBAL_FLOOD_BASE=https://api.example.com ./global-flood-cli load 1 flood

# 6. No arguments -> interactive REPL
./global-flood-cli
global-flood> load 1 flood
global-flood> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/global-flood-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/global-flood-cli load 1 flood
   ```
...
```sh
./global-flood-cli load 1 flood          # scalar shorthand for {id:1}
./global-flood-cli load '{id:1}' flood     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `global-flood>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `flood`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/global-flood-cli`. |
...
(`./global-flood-cli <expr>`) and interactively (the REPL), and expressions compose the
