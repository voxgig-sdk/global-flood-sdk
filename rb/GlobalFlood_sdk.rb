...
require_relative 'GlobalFlood_types'
...
class GlobalFloodSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = GlobalFloodHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = GlobalFloodHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, GlobalFloodFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    GlobalFloodUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = GlobalFloodHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = GlobalFloodHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = GlobalFloodHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = GlobalFloodHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = GlobalFloodError.new(
        "graphql_error", "GlobalFloodSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = GlobalFloodSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
