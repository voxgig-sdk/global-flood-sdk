...
module GlobalFloodUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = GlobalFloodResponse.new({})
    ...
    if fetch_err
      ...
      response = GlobalFloodResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = GlobalFloodResponse.new(fetched)
    else
