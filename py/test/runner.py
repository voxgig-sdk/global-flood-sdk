...
class GlobalFloodTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            GlobalFloodTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = GlobalFloodTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = GlobalFloodTestRunner.getenv("GLOBAL_FLOOD_TEST_LIVE")
        override = GlobalFloodTestRunner.getenv("GLOBAL_FLOOD_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = GlobalFloodTestRunner.getenv(key)
        ...
        explain = GlobalFloodTestRunner.getenv("GLOBAL_FLOOD_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["GLOBAL_FLOOD_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if GlobalFloodTestRunner._test_control is not None:
            return GlobalFloodTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                GlobalFloodTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            GlobalFloodTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = GlobalFloodTestRunner.load_test_control()
...
def load_env_local():
    GlobalFloodTestRunner.load_env_local()


def env_override(m):
    return GlobalFloodTestRunner.env_override(m)


def entity_data(v):
    return GlobalFloodTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return GlobalFloodTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return GlobalFloodTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return GlobalFloodTestRunner.load_test_control()


def live_client_options():
    return GlobalFloodTestRunner.live_client_options()


def live_delay_ms():
    return GlobalFloodTestRunner.live_delay_ms()
