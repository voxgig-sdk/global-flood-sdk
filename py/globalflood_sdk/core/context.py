...
from globalflood_sdk.core.helpers import get_ctx_prop, to_map
...
class GlobalFloodContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        if isinstance(sp, GlobalFloodSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, GlobalFloodResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, GlobalFloodResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def make_error(self, code, msg):
        return GlobalFloodError(code, msg, self)
