...
from globalflood_sdk.globalflood_types import (
    Flood,
    FloodLoadMatch,
)
...
class FloodEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return FloodEntity(self._client, opts)
    ...
    def load(self, reqmatch=None, ctrl=None) -> Flood:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Flood().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
