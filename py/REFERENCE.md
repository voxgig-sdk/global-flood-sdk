# GlobalFlood Python SDK Reference

Complete API reference for the GlobalFlood Python SDK.
...
#### `GlobalFloodSDK.test(testopts=None, sdkopts=None)`
...
Create a new `FloodEntity` instance. Pass `None` for no initial data.
...
| --- | --- | --- | --- |
| `daily` | `dict` | No | Daily flood data |
| `daily_units` | `dict` | No | Units for each daily variable |
...
```python
result = client.Flood().load({"latitude": "latitude", "longitude": "longitude"})
```
...
Create a new `FloodEntity` instance with the same options.
