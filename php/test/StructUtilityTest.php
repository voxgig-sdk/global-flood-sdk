...
// GlobalFlood SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = GlobalFloodOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(GlobalFloodOmni::structfix($now, Struct::undef()))
                        !== json_encode(GlobalFloodOmni::structfix($val, Struct::undef()))) {
