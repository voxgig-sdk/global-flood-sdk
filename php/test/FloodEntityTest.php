...
class FloodEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = GlobalFloodSDK::test(null, null);
        $ent = $testsdk->Flood(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = flood_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "flood." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        $flood_ref01_data = null;
        if (count($flood_ref01_data_raw) > 0) {
...
function flood_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/flood/FloodTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["flood01", "flood02", "flood03"] as $k) {
