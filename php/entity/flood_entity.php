...
class FloodEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new FloodEntity($this->_client, $opts);
    }

    /**
     * @param Flood|array $args Flood data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = GlobalFloodHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Flood|array The current Flood data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Flood fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = GlobalFloodHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Flood fields).
     */
     ...
     * Load a single Flood.
     *
     * @param FloodLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed FloodLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Flood|array The loaded Flood as an assoc-array at the
     *   SDK boundary; throws GlobalFloodError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = GlobalFloodHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
