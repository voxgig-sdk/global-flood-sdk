...
class GlobalFloodSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = GlobalFloodHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = GlobalFloodHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!GlobalFloodFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, GlobalFloodFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return GlobalFloodUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = GlobalFloodHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = GlobalFloodHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = GlobalFloodHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new GlobalFloodError($op . "_allow",
                "GlobalFloodSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = GlobalFloodHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new GlobalFloodError("graphql_error",
                "GlobalFloodSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function Flood($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_flood === null) {
                $this->_flood = new FloodEntity($this, null);
            }
            return $this->_flood;
        }
        return new FloodEntity($this, $data);
    }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new GlobalFloodSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
