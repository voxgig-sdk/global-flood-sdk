...
The PHP SDK for the GlobalFlood API — an entity-oriented client using PHP conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `$client->Flood()` — with named operations (`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```php
try {
    // load() returns the ENTITY — call data_get() for the Flood record (throws on error).
    $flood = $client->Flood()->load(["latitude" => "example_latitude", "longitude" => "example_longitude"]);
    print_r($flood->data_get());
} catch (\Throwable $err) {
...
```php
$client = GlobalFloodSDK::test($testopts, $sdkopts);
```
...
| `Flood` | `($data): FloodEntity` | Create a Flood entity instance. |
...
| --- | --- | --- |
| `daily` | `array` | Daily flood data |
| `daily_units` | `array` | Units for each daily variable |
...
```
php/
├── globalflood_sdk.php          -- Main SDK class
...
The main class (`globalflood_sdk.php`) exports the SDK class
