...
[MCP](https://modelcontextprotocol.io) server exposing the GlobalFlood SDK as
two agent tools — `global-flood_list` and `global-flood_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/global-flood-mcp)
make build

# 2. Provide credentials via the environment
export GLOBAL_FLOOD_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user global-flood \
  -- /absolute/path/to/global-flood-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./global-flood-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// global-flood_list: first page of records
{ "entity": "flood" }
{ "entity": "flood", "query": { } }

// global-flood_load: one record by id
{ "entity": "flood", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/global-flood-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user global-flood \
     -- "$PWD"/dist/*/global-flood-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `global-flood_list` and `global-flood_load` tools now appear
   in new sessions. Ask the agent to *"list flood using global-flood"*
   and it calls `global-flood_list` with `{"entity":"flood"}`.
...
```sh
export GLOBAL_FLOOD_APIKEY=sk_live_xxx            # API key
export GLOBAL_FLOOD_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `global-flood_list` tool
...
### Call the `global-flood_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `global-flood_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `global-flood_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `GLOBAL_FLOOD_APIKEY` | API key sent with every request. |
| `GLOBAL_FLOOD_BASE` | Optional override of the API base URL. |
...
```sh
./global-flood-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"global-flood_load","arguments":{"entity":"flood","query":{"id":1}}}}'
```
