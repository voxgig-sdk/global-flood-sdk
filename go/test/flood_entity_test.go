...
func TestFloodEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set GLOBAL_FLOOD_TEST_FLOOD_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		floodRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.flood")))
...
func floodBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("GLOBAL_FLOOD_TEST_FLOOD_ENTID")
	...
	idmapResolved := core.ToMapAny(env["GLOBAL_FLOOD_TEST_FLOOD_ENTID"])
