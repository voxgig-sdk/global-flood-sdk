...
import (
	...
	omni "github.com/voxgig-sdk/global-flood-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.GlobalFloodError); is {
				out := map[string]any{
					"name":    "GlobalFloodError",
...
func omniCtx(arg any, client *sdk.GlobalFloodSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.GlobalFloodSDK); is {
			client = live
