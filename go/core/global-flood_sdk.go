...
func (sdk *GlobalFloodSDK) OptionsMap() map[string]any {
...
func (sdk *GlobalFloodSDK) GetUtility() *Utility {
	return CopyUtility(sdk.utility)
}

func (sdk *GlobalFloodSDK) GetRootCtx() *Context {
	return sdk.rootctx
}

func (sdk *GlobalFloodSDK) Prepare(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *GlobalFloodSDK) Direct(fetchargs map[string]any) (map[string]any, error) {
...
func (sdk *GlobalFloodSDK) opAllowed(op string) bool {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return strings.Contains(allowOp, op)
}

func (sdk *GlobalFloodSDK) opDenied(op string) map[string]any {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return map[string]any{
		"ok": false,
		"err": fmt.Errorf("GlobalFloodSDK: %s: operation not allowed by"+
...
// gate by passing it.
func (sdk *GlobalFloodSDK) rawRequest(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *GlobalFloodSDK) Graphql(
	...
	if 0 < len(errors) {
		...
		res["ok"] = false
		res["err"] = fmt.Errorf("GlobalFloodSDK: graphql: %s", msg)
		res["graphql"] = errors
	}
