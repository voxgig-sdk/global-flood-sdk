# GlobalFlood Golang SDK Reference

Complete API reference for the GlobalFlood Golang SDK.
...
#### `TestSDK(testopts, sdkopts map[string]any) *GlobalFloodSDK`
...
#### `Flood(data map[string]any) GlobalFloodEntity`

Create a new `Flood` entity instance. Pass `nil` for no initial data.
...
```go
result, err := client.Flood(nil).Load(map[string]any{"latitude": "latitude", "longitude": "longitude"}, nil)
if err != nil {
    panic(err)
}
fmt.Println(result)
