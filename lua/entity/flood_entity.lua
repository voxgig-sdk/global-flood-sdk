...
local FloodEntity = {}
FloodEntity.__index = FloodEntity


function FloodEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, FloodEntity)
  self._name = "flood"
  self._client = client
...
function FloodEntity:get_name()
  return self._name
end


function FloodEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return FloodEntity.new(self._client, opts)
end
...
function FloodEntity:mark_deleted()
  self._deleted = true
end


function FloodEntity:deleted()
  return true == self._deleted
end


function FloodEntity:data_set(args)
  if args ~= nil then
...
function FloodEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function FloodEntity:match_set(args)
  if args ~= nil then
...
function FloodEntity:match_get()
...
function FloodEntity:stream(action, args, callopts)
...
function FloodEntity:_run_op(ctx, post_done)
  local utility = self._utility
